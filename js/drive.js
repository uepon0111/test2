...
const Drive = (() => {
  ...
  async function _diffAudio(localTracks, deletedIds) {
    ...
    // Map: filename base (= our trackId when we uploaded) → drive file
    const driveByBase = new Map(
      driveFiles.map(f => [f.name.replace(/\.[^.]+$/,''), f])
    );
    const localIdSet = new Set(localTracks.map(t => t.id));
    ...
    try {
      ...
      for (const id of remoteDelIds) {
        ...
        if (t && !deletedIds.has(id)) {
          // Another device deleted this — remove locally (blobs + logs + playlists)
          await Storage.deleteTrack(id);
          // Ensure it's in our deletedIds so we don't re-import it
          deletedIds.add(id);
        }
      }
      ...
      _prog(40, 'ローカル削除を同期中...');
      const allLocalTracks = await Storage.getTracks();
      // Find tracks we know about from remote but locally deleted
      for (const id of deletedIds) {
        // Check if there's a drive file we need to clean up
        // (we might not have the driveFileId anymore if already deleted locally)
        // Try to find it in the remote index
