...
const Player = (() => {
  ...
  function syncQueue(trackIds) {
    const currentId = getCurrentTrackId();
    _origQueue = [...trackIds];

    if (_shuffle) {
      const base = _shuffleList(trackIds, currentId);
      _queue = base;
      _currentIdx = currentId && _queue.includes(currentId) ? _queue.indexOf(currentId) : -1;
    } else {
      _queue = [...trackIds];
      _currentIdx = currentId && _queue.includes(currentId) ? _queue.indexOf(currentId) : -1;
    }
  ...
  /* ─────────────────────────────────────────
     SHUFFLE
  ───────────────────────────────────────── */
  function _shuffleList(list, keepId = null) {
    const hasKeep = keepId != null && list.includes(keepId);
    const rest = hasKeep ? list.filter(id => id !== keepId) : [...list];
    for (let i = rest.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [rest[i], rest[j]] = [rest[j], rest[i]];
    }
    return hasKeep ? [keepId, ...rest] : rest;
  }

  function _shuffleQueue() {
    const current = getCurrentTrackId();
    _queue = _shuffleList(_queue, current);
    _currentIdx = current ? 0 : (_queue.length ? -1 : 0);
  }
  ...
  /* ─────────────────────────────────────────
     CONTROLS
  ───────────────────────────────────────── */
  function _getFallbackTrackIds() {
    if (typeof UI !== 'undefined' && UI.getVisibleTrackIds) {
      const ids = UI.getVisibleTrackIds();
      if (Array.isArray(ids) && ids.length > 0) return ids;
    }
    return [];
  }

  function _hasSelectedTrack() {
    return _currentIdx >= 0 && !!getCurrentTrackId();
  }

  function togglePlay() {
    if (audio.paused) {
      if (!_hasSelectedTrack()) {
        const ids = _queue.length > 0 ? [..._queue] : _getFallbackTrackIds();
        if (ids.length > 0) setQueue(ids, 0);
        return;
      }
      if (audio.src) {
        audio.play();
