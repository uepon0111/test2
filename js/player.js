...
const Player = (() => {
  ...
  function _flushLog() {
    ...
    clearInterval(_logFlushTimer);
    _logFlushTimer = null;
    if (_loggedTrackId && _playedSeconds > 3) {
  ...
  function _updateVolumeUI() {
    const icon = document.getElementById('pw-vol-icon');
    const fpoIcon = document.getElementById('fpo-vol-icon');
