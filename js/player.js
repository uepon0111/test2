...
const Player = (() => {
  ...
  /**
   * Update queue order to match the current display order
   * without interrupting playback or reloading the current track.
   * Called by UI whenever sort order or manual reorder changes.
   */
  function updateQueueOrder(trackIds) {
    const currentId = getCurrentTrackId();
    _origQueue  = [...trackIds];
    if (_shuffle) {
      // Re-shuffle preserving current track at front
      _queue = [...trackIds];
      _shuffleQueue();
    } else {
      _queue = [...trackIds];
      _currentIdx = currentId ? _queue.indexOf(currentId) : -1;
    }
    _saveState();
  }
