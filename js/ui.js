...
const UI = (() => {
  ...
  async function renderEditGrid() {
    ...
    grid.style.setProperty('--edit-cols', isPortrait ? '2' : String(_editCols));
  ...
  function toggleEditCols() {
    // On mobile CSS forces 2 columns — toggle has no effect.
    if (window.matchMedia('(max-width:768px)').matches) return;
    _editCols = (_editCols === 4) ? 6 : 4;
    const grid = document.getElementById('edit-grid');
    if (grid) grid.style.setProperty('--edit-cols', String(_editCols));
  }
