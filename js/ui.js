...
const UI = (() => {
  ...
  function _applyEditGridLayout() {
    const grid = document.getElementById('edit-grid');
    if (!grid) return;

    const isPortrait = window.matchMedia('(orientation: portrait)').matches;
    const cols = isPortrait ? 2 : (_editCols === 6 ? 6 : 4);

    grid.dataset.mode = isPortrait ? 'portrait' : 'landscape';
    grid.dataset.cols = String(cols);
    grid.style.setProperty('--edit-cols', String(cols));
    grid.style.gridTemplateColumns = `repeat(${cols}, minmax(0, 1fr))`;

    if (isPortrait) {
      grid.style.setProperty('--edit-card-pad', 'clamp(5px, 1.0vw, 8px)');
      grid.style.setProperty('--edit-card-gap', 'clamp(2px, 0.45vw, 4px)');
      grid.style.setProperty('--edit-title-size', 'clamp(10px, 1.55vw, 12px)');
      grid.style.setProperty('--edit-artist-size', 'clamp(9px, 1.25vw, 11px)');
      grid.style.setProperty('--edit-date-size', 'clamp(8px, 1.05vw, 10px)');
      grid.style.setProperty('--edit-tag-size', 'clamp(8px, 1.0vw, 9px)');
    } else if (cols === 6) {
      grid.style.setProperty('--edit-card-pad', 'clamp(5px, 0.55vw, 8px)');
      grid.style.setProperty('--edit-card-gap', 'clamp(2px, 0.28vw, 3px)');
      grid.style.setProperty('--edit-title-size', 'clamp(10px, 0.7vw, 11px)');
      grid.style.setProperty('--edit-artist-size', 'clamp(9px, 0.63vw, 10px)');
      grid.style.setProperty('--edit-date-size', 'clamp(8px, 0.58vw, 9px)');
      grid.style.setProperty('--edit-tag-size', 'clamp(8px, 0.56vw, 9px)');
    } else {
      grid.style.setProperty('--edit-card-pad', 'clamp(6px, 0.8vw, 9px)');
      grid.style.setProperty('--edit-card-gap', 'clamp(2px, 0.35vw, 4px)');
      grid.style.setProperty('--edit-title-size', 'clamp(11px, 0.8vw, 12px)');
      grid.style.setProperty('--edit-artist-size', 'clamp(10px, 0.72vw, 11px)');
      grid.style.setProperty('--edit-date-size', 'clamp(9px, 0.68vw, 10px)');
      grid.style.setProperty('--edit-tag-size', 'clamp(9px, 0.65vw, 10px)');
    }
  }
  ...
  function applySort(syncPlayer = false) {
    const sortSel = document.getElementById('pl-sort-select');
    if (sortSel) _sortKey = sortSel.value;
    ...
    _updateTrackCount();
    if (_playlistVL) {
      _playlistVL.setItems(_displayTracks.slice());
      _playlistVL.refresh();
    }

    if (sortSel) sortSel.value = _sortKey;

    if (syncPlayer && typeof Player !== 'undefined' && Player.syncQueue) {
      Player.syncQueue(_displayTracks.map(t => t.id));
    }
  }
  ...
  function getVisibleTrackIds() {
    return _displayTracks.map(t => t.id);
  }
  ...
  async function _moveTrack(id, dir, e) {
    ...
    if (typeof Player !== 'undefined' && Player.syncQueue) Player.syncQueue(_displayTracks.map(t => t.id));
    _updateTrackCount();
  ...
  async function renderEditGrid() {
    ...
    _editTracks = list;
    _applyEditGridLayout();

    grid.innerHTML = list.map(t => {
      const thumbInner = t._thumbUrl
        ? `<img src="${t._thumbUrl}" alt="${_esc(t.title)}" loading="lazy">`
        : '<i class="fa-solid fa-music"></i>';

      const releaseDate = t.releaseDate ? _esc(t.releaseDate) : '—';
      ...
      const extraTags = (t.tags || []).length > 3
        ? `<span class="edit-tag-more">+${(t.tags || []).length - 3}</span>` : '';
      ...
      return `<div class="edit-card" onclick="UI.openEditTrackModal('${t.id}',event)">
        ...
        <div class="edit-card-info">
          <div class="edit-card-title" title="${_esc(t.title || '')}">${_esc(t.title || '無題')}</div>
          <div class="edit-card-artist" title="${_esc(t.artist || '')}">${_esc(t.artist || '—')}</div>
          <div class="edit-card-date"><i class="fa-regular fa-calendar"></i><span>${releaseDate}</span></div>
  ...
  function init() {
    ...
    const syncEditGridLayout = () => {
      if (document.getElementById('edit-grid')) _applyEditGridLayout();
    };
    window.addEventListener('resize', syncEditGridLayout, { passive: true });
    window.addEventListener('orientationchange', syncEditGridLayout);
  }
