...
const UI = (() => {
  ...
  class VirtualList {
    constructor(options) {
      this.outer    = options.outer;    // scroll container el
      this.inner    = options.inner;    // total height el
      this.viewport = options.viewport; // items container el
    ...
    _render() {
      ...
      const viewH      = this.outer.clientHeight;
      const start      = Math.max(0, Math.floor(scrollTop / this.itemH) - this.buffer);
      const end        = Math.min(this._items.length, Math.ceil((scrollTop + viewH) / this.itemH) + this.buffer);
  ...
  async function _moveTrack(id, dir, e) {
    ...
    // Swap in display array
    [_displayTracks[idx], _displayTracks[newIdx]] = [_displayTracks[newIdx], _displayTracks[idx]];

    // Persist new order
    const orderedIds = _displayTracks.map(t => t.id);
    await Storage.reorderTracks(orderedIds);

    // Reload tracks with new order
    _tracks = await Storage.getTracks();
    await _attachThumbUrls(_tracks);
    applySort();
  }
    ...
    if (grid) {
      grid.innerHTML = `
        <div class="stat-card">
          ...
          <div class="stat-value">${totalH}<span class="stat-unit">時間</span>${totalMin}<span class="stat-unit">分</span></div>
  ...
  function _renderBarChart(tracks, logs) {
    ...
    chart.innerHTML = data.map((v, i) => {
      ...
      return `<div class="bar-col">
        <div class="bar-col-bar${pct === 100 ? ' highlight' : ''}" style="height:${pct}%"></div>
  ...
  function _initTagInput() {
    ...
    input.addEventListener('keydown', e => {
      ...
      if (existing) {
        ...
        // Create new tag on-the-fly (default color)
        Storage.createTag(val).then(async tag => {
          _tags = await Storage.getTags();
